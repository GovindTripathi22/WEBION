...
class PipelineService:
    ...
    async def process_virtual_tryon(self, 
        ...
        """Process virtual try-on request with enhanced fit analysis"""
        request_id = str(uuid.uuid4())
        ...
        if garment_result.success and pose_result.success:
            g_data = garment_result.data
            p_data = pose_result.data
            
            # 1. Alignment score: check horizontal center alignment
            g_center_x = (g_data.bbox.x1 + g_data.bbox.x2) / 2
            p_center_x = (p_data.bbox.x1 + p_data.bbox.x2) / 2
            alignment_score = max(0.5, 1.0 - abs(g_center_x - p_center_x))
            
            # 2. Fit score & Size estimation
            fit_score = 0.85
            size = "M"
            
            p_measurements = p_data.body_measurements
            if p_measurements and "shoulder_width" in p_measurements:
                sw = p_measurements["shoulder_width"]
                # Heuristic sizing based on normalized shoulder width
                if sw > 0.45: size = "XL"
                elif sw > 0.38: size = "L"
                elif sw < 0.28: size = "S"
                
                # Fit score increases if pose is "good"
                if p_data.pose_quality == "good":
                    fit_score += 0.1
            
            try_on_result = TryOnResult(
                result_image="placeholder_processed_tryon_image",
                alignment_score=min(1.0, alignment_score),
                fit_score=min(1.0, fit_score),
                size=size,
                confidence=min(g_data.confidence, p_data.confidence),
                processing_time=garment_result.total_time + pose_result.total_time,
                metadata={
                    "garment_type": g_data.type,
                    "measurements_used": p_measurements
                }
            )
