...
@router.post("/scan-garment")
async def scan_garment(file: UploadFile = File(...)):
    """Quick scan for garment metadata during live shopping"""
    if not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="File must be an image")
    ...
    try:
        # Use existing garment service for detection
        result = await garment_service.process_garment_image(content)
        
        if not result:
            raise HTTPException(status_code=404, detail="No garment detected")
            
        # Add recommended size heuristic based on area ratio
        area_ratio = result.dimensions.get("area_ratio", 0.5)
        recommended_size = "M"
        if area_ratio > 0.6: recommended_size = "XL"
        elif area_ratio > 0.45: recommended_size = "L"
        elif area_ratio < 0.25: recommended_size = "S"

        return {
            "type": result.type,
            "color": result.features.color_name,
            "bounding_box": {
                "x1": result.bbox.x1,
                "y1": result.bbox.y1,
                "x2": result.bbox.x2,
