...
export default function PhotoWizard() {
  ...
  // Tryon Store (for onboarding and live garment)
  const { openPhotoOnboarding, selectedLiveGarment } = useTryonStore();
  ...
  // Sync live garment from Tryon Store to VTON Store
  useEffect(() => {
    if (selectedLiveGarment && !garment.file && step === 'PATH_SELECT') {
      const loadLiveGarment = async () => {
        try {
          const response = await fetch(selectedLiveGarment.imageUrl);
          const blob = await response.blob();
          const file = new File([blob], `live-${selectedLiveGarment.id}.jpg`, { type: 'image/jpeg' });
          
          await setGarmentFile(file);
          // Auto-select path if garment loaded
          setPath('NORMAL');
          toast.success('Live garment loaded for high-quality try-on');
        } catch (err) {
          console.error('Failed to sync live garment to Photo Mode:', err);
        }
      };
      loadLiveGarment();
    }
  }, [selectedLiveGarment, garment.file, step, setGarmentFile, setPath]);
