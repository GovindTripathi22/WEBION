    ...
    <div className="w-full h-full p-4 space-y-6 overflow-y-auto">
      {isLiveMode && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 flex items-center gap-3">
          ...
          <div className="ml-auto text-[10px] text-red-500/70 font-medium">
            Gallery Locked
          </div>
        </div>
      )}
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {garments.slice(0, 6).map((garment) => (
            <button
              key={garment.id}
              onClick={() => !isLiveMode && selectGarment(garment.id)}
              disabled={isLiveMode}
              className={`relative aspect-square rounded-md border-2 overflow-hidden transition-all ${
                !isLiveMode && 'hover:scale-105'
              } ${
                selectedGarmentId === garment.id
                  ? 'border-primary ring-2 ring-primary/20'
                  : 'border-border hover:border-primary/50'
              } ${isLiveMode && selectedGarmentId !== garment.id ? 'opacity-30 grayscale' : ''}`}
            >
              <Image
                src={garment.src}
                alt={garment.name}
          ...
          >
            {uploading ? (
              ...
              <>
                ...
                {isLiveMode ? 'Upload Locked' : 'Add Garment'}
              </>
            )}
          </Button>

          {selectedGarment && selectedGarment.id.startsWith('custom-') && !isLiveMode && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => {
      ...
      <section className="space-y-2">
        <Button
          variant="outline"
          onClick={clearAll}
          disabled={!selectedGarment || isLiveMode}
          className="w-full"
        >
          <Trash2 className="h-4 w-4 mr-2" />
          {isLiveMode ? 'Live - Cannot Clear' : 'Clear All'}
        </Button>
      </section>
    </div>
  );
