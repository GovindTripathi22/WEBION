...
export function usePoseDetection(
  ...
  const smoothedWorldLandmarksRef = useRef<PoseLandmark[] | null>(null);
    ...
    if (currentTime !== lastVideoTimeRef.current) {
      ...
      try {
        ...
        if (result.landmarks && result.landmarks.length > 0) {
          ...
          const rawWorldLandmarks = result.worldLandmarks ? result.worldLandmarks[0] : [];

          // Calculate overall confidence from visible landmarks
          const visibleLandmarks = rawLandmarks.filter(l => (l.visibility || 0) > 0.5);
          const confidence = visibleLandmarks.length / rawLandmarks.length;
          ...
          const mirroredWorldLandmarks = rawWorldLandmarks.map(l => ({
          ...
          // Apply exponential smoothing to reduce jitter (tracing improvement)
          const alpha = 0.35; // Smoothing factor (lower = smoother but slightly more lag, 0.35 is optimal)

          let smoothedLandmarks: PoseLandmark[] = [];
          if (!smoothedLandmarksRef.current || smoothedLandmarksRef.current.length !== mirroredLandmarks.length) {
            smoothedLandmarks = mirroredLandmarks;
          } else {
            smoothedLandmarks = mirroredLandmarks.map((l, idx) => {
              const prev = smoothedLandmarksRef.current![idx];
              if ((l.visibility || 0) < 0.3) return prev; // Keep previous if confidence is too low
              return {
                x: alpha * l.x + (1 - alpha) * prev.x,
                y: alpha * l.y + (1 - alpha) * prev.y,
                z: alpha * l.z + (1 - alpha) * prev.z,
                visibility: l.visibility
              };
            });
          }
          ...
          let smoothedWorldLandmarks: PoseLandmark[] = [];
          if (!smoothedWorldLandmarksRef.current || smoothedWorldLandmarksRef.current.length !== mirroredWorldLandmarks.length) {
            smoothedWorldLandmarks = mirroredWorldLandmarks;
          } else {
            smoothedWorldLandmarks = mirroredWorldLandmarks.map((l, idx) => {
              const prev = smoothedWorldLandmarksRef.current![idx];
              if ((l.visibility || 0) < 0.3) return prev; // Keep previous if confidence is too low
              return {
                x: alpha * l.x + (1 - alpha) * prev.x,
                y: alpha * l.y + (1 - alpha) * prev.y,
          ...
          smoothedWorldLandmarksRef.current = smoothedWorldLandmarks;
